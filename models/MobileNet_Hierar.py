...
@register_model('my_mobilenet_v3_large')
...
class HierarchicalMobileNetV3(nn.Module):
    def __init__(self, num_classes: int = 14, pretrained=False):
        super(HierarchicalMobileNetV3, self).__init__()
        ...
        # Split the features into layers
        self.layer1 = self.features[:4]
        self.layer2 = self.features[4:7]
        self.layer3 = self.features[7:]

        self.attn = SelfAttention(960)
        
        self.avgpool_branch = nn.AdaptiveAvgPool2d((1, 1))
        self.fc_branch = nn.Linear(960, 2)
    ...
    def _forward_impl(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x_branch = self.layer3(x)
        x_branch = self.attn(x_branch)

        x = self.layer3(x)
        x = torch.cat((x, x, x_branch), dim=1)

        x_branch = self.avgpool_branch(x_branch)
        x_branch = x_branch.view(x_branch.size(0), -1)
        x_branch = self.fc_branch(x_branch)
